...
# Step 3: Define Dataset class for autoencoder
print("Defining AutoEncoderTrainingDataset...")
class AutoEncoderTrainingDataset(Dataset):
    def __init__(self, csv_path, device):
        print(f"Loading CSV for AutoEncoder: {csv_path}")
...
# Step 4: Training loop for autoencoder
print("Defining autoencoder training loop...")
def pretrain_autoencoder(model, dataloader, num_epochs, optimizer, criterion, device, test_dataloader=None):
    ...
    for epoch in range(num_epochs):
        ...
        for ii, (inputs, targets) in enumerate(dataloader):
            print(f"Batch {ii + 1}/{len(dataloader)}")
...
print("Preparing and sampling training dataset...")
...
print(f"Saved balanced sampled dataset to {sampled_csv_file}")
...
# Step 8: Freeze encoder
print("Freezing encoder...")
for param in autoencoder.encoder.parameters():
    param.requires_grad = False

# Step 9: Define classifier
print("Defining MLP classifier...")
...
# Step 10: Define classifier dataset
print("Defining AutoEncodedDataset...")
class AutoEncodedDataset(Dataset):
    def __init__(self, csv_path, ae_model, device):
        print(f"Loading encoded dataset from: {csv_path}")
        self.df = pd.read_csv(csv_path)
...
# Step 11: Load dataset and sampling method
print("Setting up classifier training dataset and sampler...")
